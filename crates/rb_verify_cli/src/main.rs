...
//!
//! Thin CLI over `rb_verify_cli`'s two scoring functions — argument parsing
//! and human-readable output only, no logic of its own (see `lib.rs` for
//! why the actual wiring lives there instead). Two modes:
...
use rb_verify_cli::{
    score_capture_against_candidate, score_replay_against_capture, DEFAULT_MAX_TIMESTAMP_DELTA_SECS,
};
use std::env;
