...
//! (throttle, steering), boost, handbrake, a ground jump, a double jump
//! (plain or a directional dodge, depending on stick input), a wall jump,
//! and airborne pitch/yaw/roll (air control) on a car — see its own module
//! doc for what's deliberately still out of scope (a dodge variant of the
//! wall jump, variable jump height). `PhysicsWorld` gains arena walls
//! (`with_wall`) as generic flat `StaticPlane` geometry every body collides
//! with, the same way it already collides with the ground.
//!
