...
//! (throttle, steering), boost, handbrake, a ground jump, a double jump, a
//! wall jump, and airborne pitch/yaw/roll (air control) on a car — see its
//! own module doc for what's deliberately still out of scope (the dodge
//! directional impulse/torque a real double jump pairs with, variable jump
//! height). `PhysicsWorld` gains arena walls (`with_wall`) as generic flat
