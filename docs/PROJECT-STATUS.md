...
- `RB-PHYSICS-001-FR-077` (wire candidate engine into `rb_verify_cli`,
  implemented; real-capture run pending): `rb_verify_cli` gains
  ...
  `test` green (388 tests). The one manual run this requirement's own
  ...
  environment this sandbox doesn't have. `RB-PHYSICS-001-FR-005`
  (real-data constant calibration) still doesn't start until that run
  happens and produces a first real number.
...
- `RB-PHYSICS-001-FR-077`'s one manual run against the real capture from
  `RB-VERIFY-002-FR-001` (`cargo run -p rb_verify_cli -- --self
  <real-capture-file>`) — the wiring is implemented and tested, but this
  sandbox has no real Rocket League/BakkesMod environment or the real
  capture file itself; needs the owner to run it and report the resulting
  numbers back.
...
1. (Optional, owner-side, non-blocking) The manual BakkesMod-overlay
   ...
   Blocked).
2. (Owner-side, blocking `FR-005`) `RB-PHYSICS-001-FR-077`'s wiring is
   implemented and tested; the immediate next step is its one manual run
   against the real capture from `RB-VERIFY-002-FR-001` (see Blocked),
   producing this project's first genuine fidelity number.
   `RB-PHYSICS-001-FR-005` (real-data constant calibration) doesn't start
