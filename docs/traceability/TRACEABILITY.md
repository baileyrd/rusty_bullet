...
| `RB-VERIFY-001-FR-001..003` | `PHASE-0-REPLAY-INGEST` | `rb_domain::port::PhysicsStateSource`, `boxcars`+`subtr-actor` (see Cargo.toml comment) | `crates/rb_replay_ingest/src/{lib,convert}.rs` | `rb_replay_ingest` unit tests (10), incl. real-fixture integration test | replay-ingestion commit | Implemented, Verified (real fixture; owner-data cross-check still open) |
