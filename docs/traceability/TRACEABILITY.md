...
| `RB-PHYSICS-001-FR-001..003` | `PHASE-1-PHYSICS-CORE-V0` | ADR-0003 (fidelity target), ADR-0004 (Bullet3 port) | `crates/rb_physics_bullet` (`integrate`, `collision`, `solver`, `world`) | `rb_physics_bullet` unit tests (65, incl. sphere-path regression coverage) | bullet3-port + car-box + ball-vs-car + box-vs-box + multi-car commits | Implemented, Verified (v0 scope; not yet scored against real data) |
