...
| `RB-PHYSICS-001-FR-001..003` | `PHASE-1-PHYSICS-CORE-V0` | ADR-0003 (fidelity target), ADR-0004 (Bullet3 port) | `crates/rb_physics_bullet` (`integrate`, `collision`, `solver`, `world`) | `rb_physics_bullet` unit tests (47, incl. sphere-path regression coverage) | bullet3-port + car-box commits | Implemented, Verified (v0 scope; not yet scored against real data) |
| `RB-PHYSICS-001-FR-004` (car boxes, general inertia, multi-contact) | `PHASE-1-PHYSICS-CORE` | `RigidBody`/`Shape` (body.rs), `Mat3` (mat3.rs) | `crates/rb_physics_bullet` (`body`, `mat3`, `collision`, `solver`, `world`) | `rb_physics_bullet` unit tests (21 new: inertia, box-vs-plane contact counts, multi-contact settling) | car-box commit | Implemented, Verified (unit tests only; box-vs-sphere collision not implemented, not yet scored against real data) |
