...
| `RB-PHYSICS-001` | Physics Core Port | 0.22.0 | In Progress | In Progress (sphere + box bodies, ball-vs-car, car-vs-car, body-vs-arena-wall, and ball-vs-curved-fillet collision wired into a multi-car `PhysicsWorld`, ground-driving car input, boost, handbrake, a variable-height ground jump, a double jump (plain or a flip-cancelable dodge), a wall jump (itself dodgeable), air control, a gentle landing auto-orientation assist, a modeled octagonal arena footprint, and curved fillets throughout the arena's vertical boundary (floor/ceiling seams for all 9 walls, plus all 8 corner-wall vertical edges) for the ball; FR-005 open, car-vs-curve/goal cutouts open) | Verified (unit tests; not yet scored against real data) | RB-VERIFY-003 | baileyrd | [spec](physics/RB-PHYSICS-001-physics-core-port.md) | `crates/rb_physics_bullet` tests (181) |
