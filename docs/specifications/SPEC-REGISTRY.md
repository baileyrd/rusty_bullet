...
| `RB-PHYSICS-001` | Physics Core Port | 0.26.0 | In Progress | In Progress (sphere + box bodies, ball-vs-car, car-vs-car, body-vs-arena-wall, and ball-vs-curved-fillet collision wired into a multi-car `PhysicsWorld`, ground-driving car input, boost, handbrake, a variable-height ground jump, a double jump (plain or a flip-cancelable dodge), a wall jump (itself dodgeable), air control, a gentle landing auto-orientation assist, a modeled octagonal arena footprint, curved fillets throughout the arena's vertical boundary (floor/ceiling seams for all 9 walls, all 8 corner-wall vertical edges, and all 16 compound-corner spherical patches — the 4 corner walls' own floor/ceiling-seam arches and all 16 compound-corner fillets now sized with a distinctly larger, dedicated radius since FR-025), and an actual goal-mouth window (with its own 3 rounded edges) cut into each back wall for the ball, with the goal's own remaining post-crossbar compound corners now rounded by 4 more corner fillets since FR-026 (20 compound-corner fillets total); FR-005 open, car-vs-curve/car-into-goal/goal-interior open) | Verified (unit tests; not yet scored against real data) | RB-VERIFY-003 | baileyrd | [spec](physics/RB-PHYSICS-001-physics-core-port.md) | `crates/rb_physics_bullet` tests (215) |
