...
| `RB-PHYSICS-001` | Physics Core Port | 0.18.0 | In Progress | In Progress (sphere + box bodies, ball-vs-car, car-vs-car, and body-vs-arena-wall collision wired into a multi-car `PhysicsWorld`, ground-driving car input, boost, handbrake, a variable-height ground jump, a double jump (plain or a flip-cancelable dodge), a wall jump (itself dodgeable), air control, and a gentle landing auto-orientation assist; FR-005 open, full-arena-footprint open) | Verified (unit tests; not yet scored against real data) | RB-VERIFY-003 | baileyrd | [spec](physics/RB-PHYSICS-001-physics-core-port.md) | `crates/rb_physics_bullet` tests (143) |
