...
| `PHASE-0-EXIT` | End-to-end verification pipeline: real replay + real capture both score via `rb_verify_cli`. | `PHASE-0-REPLAY-INGEST`, `PHASE-0-CAPTURE-INGEST` | RB-VERIFY-003 | Pipeline runs end-to-end and produces a divergence score on ≥1 real replay and ≥1 real BakkesMod capture. The mechanical half is met: `rb_verify_cli::score_replay_against_capture` runs the full ingest-then-score pipeline against a real vendored replay fixture and a capture file without erroring, ball and car scoring both included (5 frames compared, 5 car pairs matched by `player_id`). Not fully met: that capture is synthetic, not a real BakkesMod recording (blocked on `PHASE-0-CAPTURE-INGEST`/`RB-VERIFY-002-FR-001`), and the score itself isn't a meaningful fidelity number (unrelated matches; `RB-VERIFY-003-FR-003`, timestamp-tolerant alignment, still open). | In Progress | `crates/rb_domain`, 8 car-scoring tests + `crates/rb_verify_cli`, 3 tests + manual run |
