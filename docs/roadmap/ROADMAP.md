...
| `PHASE-1-PHYSICS-CORE` | Extend to car (box) rigid bodies, general inertia tensors, multi-contact resolution, ball-vs-car collision; calibrate constants against real recorded data. Est. 2-4 months part-time total for Phase 1. | `PHASE-1-PHYSICS-CORE-V0`, `PHASE-0-EXIT` | RB-PHYSICS-001 | Divergence score against recorded ground truth reaches a threshold calibrated from the first real scoring run (see RB-VERIFY-003 open questions). Box bodies, general 3x3 inertia, multi-contact resolution, and sphere-vs-box (ball-vs-car) collision are all implemented and unit-tested (`RB-PHYSICS-001-FR-004`, complete — box-vs-box collision remains open, but doesn't block this gate since this scope has only one car). Not met: constant calibration (`FR-005`) needs real scoring data, which needs `PHASE-0-EXIT` to land first. | In Progress | `crates/rb_physics_bullet`, 32 new tests (58 total) |
