...
| `PHASE-0-REPLAY-INGEST` | `rb_replay_ingest` parses real `.replay` files into `PhysicsFrame`s (via `boxcars` + `subtr-actor`). | `PHASE-0-BOOTSTRAP` | RB-VERIFY-001 | RB-VERIFY-001 acceptance criteria met on ≥1 real replay. Met against a vendored third-party fixture (12,029 frames, sane bounds); the owner-data cross-check criterion is still open — no owner replay file available in this environment. | In Progress | `crates/rb_replay_ingest`, 10 tests |
